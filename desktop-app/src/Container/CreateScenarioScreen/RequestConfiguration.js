...
const RequestConfiguration = () => {
  const [requestConfiguration, setConfiguration] = useState([{ port: "", path: "", host: "", method: "" }])
  ...
  const addMoreRequest = () => {
    setConfiguration([...requestConfiguration, { port: "", path: "", host: "", method: "" }])
  }
        ...
        requestConfiguration.map(
          (requestconfig, requestIndex) =>
            <div key={requestIndex} style={{ width: "100%", display: "flex", flexWrap: "wrap" }}>
              ...
              {
                requestoptions.map((option, index) => <CustomInput key={option.name} inputname={option.name} label={option.label} onChange={onChange} placeholder={option.placeholder} requestNumber={requestIndex} value={requestconfig[option.name]} />)
              }
            </div>)
      }
      <div style={{ width: "100%", display: "flex" ,justifyContent:"flex-end"}}>
        <Link to="/home"><button style={{cursor:"pointer",margin:10, height: "4vh", border: "none", borderRadius: 5, backgroundColor: "#3859CC", color: "#ffffff", fontSize: "1.3vw" }}>Create</button></Link>
      </div>
    </div>
  );
}
