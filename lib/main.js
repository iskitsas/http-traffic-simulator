...
var numberOfTotalRequestsPerClient=gh.args(-1);//-1 for infinitive
var maxSockets=gh.args(10000);

var testDuration=gh.args(4)//-1 for infinite run, sec otherwise, eg 120 for 2 minutes test
var delayBetweenRequests=gh.args(2.9);//-1 for no delay [sec]
var randomDelayBetweenRequests=gh.args('0.9-2.9');
var throttleRequests_bps=gh.args(50000);//bytes per seconds, if -1 => no throttling | 50000bps=400kbps
var isPipelinedRequest=true;

var cleanUpOnExit=gh.args(false);
...
function start(cb,generateRequest,cleanUpOnExitCallback,sendWorkerStatsCallback,workerBackOffPolicyCallback,countersStopCondition) {
    ...
    if (cluster.isMaster) {
        ...
        function aggregateAllCounters(wm){
            ...
            for(var i=0;i<len;i++){
                ...
                if(cleanUpOnExit){
                    if(cleanUpOnExitCallback){
                        stats=cleanUpOnExitCallback(wm[key],stats);
                    }
                }
            }
        ...
        if(cb){
            var intId=setInterval(function(){
                if(Object.keys(workerMessages).length==workers){
                    cb(aggregateAllCounters(workerMessages));
                    clearInterval(intId);
                    //process.exit();
                }
            },1000);
                ...
                if(numberOfTotalRequestsPerWorker>0){
                    ...
                    setInterval(function(){
                        ...
                        if(receivedStopSignal){
                            ...
                            sendWorkersStats(id,sendWorkerStatsCallback);
                            process.exit();
                        }else{
                            if(countersStopCondition!=null){
                                if(countersStopCondition(countObj)){
        ...
        if(workerBackOffPolicyCallback){
            workerBackOffPolicyCallback(id);
        }
...
exports.cleanUpOnExit = function(value){cleanUpOnExit=value};
...
exports.getCleanUpOnExit = function(){return cleanUpOnExit};
